...
// There are two difficulty levels in the game:
// DifficultyLevel::Easy and DifficultyLevel::Hard.
...
// In easy mode, the Program randomly selects between 1 and the maximum number of pebbles that can be removed in each round.
// In hard mode, in order to ensure that the Program achieves ultimate victory as much as possible,
// it is necessary to make the Program obtain the specified pebble number as much as possible each time.

// O: The current reached pebble serial number,
// A: This program selects the specified pebble serial number as much as possible,
// B: The remaining number of pebbles,
// C: Maximum subtractable quantity per round.
// Formula: A - O = B % (C + 1)
...
pub extern "C" fn handle() {
    ...
    match load_action_msg {
        ...
        PebblesAction::Turn(mut x) => {
            ...
            // The reason for writing this is because in practice,
            // It has been found that the same `payload` can only be used once in a branch using `msg.reply`
            if peb_rem == 0 {
